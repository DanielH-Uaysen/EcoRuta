...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* ---------- CUERPO ---------- */
        .cuerpo { display: grid; grid-template-columns: 320px 1fr; min-height: 0; }
        ...
        /* ---------- PANEL DERECHO: CATEGORÍAS (deslizante, cerrado por defecto) ---------- */
        .sidebar-der {
            position: absolute; top: 0; right: 0; bottom: 0; width: 280px; max-width: 85%;
            border-right: none; border-left: 1px solid #e3eaef;
            box-shadow: -10px 0 28px rgba(0,0,0,0.16);
            transform: translateX(105%); transition: transform 0.3s cubic-bezier(.2,.7,.2,1);
            z-index: 1100;
        }
        .sidebar-der.abierto { transform: translateX(0); }
        .pm-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
        .pm-head .titulo-sec { margin: 0; }
        .pm-cerrar { background: #eef3f6; border: none; color: var(--gris); width: 30px; height: 30px; border-radius: 50%; cursor: pointer; font-size: 0.9rem; transition: 0.2s; }
        .pm-cerrar:hover { background: #ffebee; color: #c62828; }
        .btn-categorias {
            position: absolute; top: 14px; right: 14px; z-index: 1050;
            display: flex; align-items: center; gap: 8px;
            background: linear-gradient(135deg, var(--verde), var(--verde-osc)); color: #fff;
        ...
        .btn-categorias:hover { transform: translateY(-2px); }
        .btn-categorias.oculto { opacity: 0; pointer-events: none; }
        .btn-categorias .punto-activo { width: 9px; height: 9px; border-radius: 50%; background: var(--amarillo); display: none; }
        .btn-categorias .punto-activo.on { display: inline-block; }
        ...
        /* ---------- MAPA ---------- */
        .mapa-wrap { position: relative; min-height: 0; z-index: 400; }
        ...
        @media (max-width: 860px) {
            ...
            .sidebar-der { display: block; } /* el panel deslizante sí funciona en móvil */
        }
    </style>
</head>
<body>
<div class="app">
    ...
    <div class="cuerpo">
        ...
        <div class="mapa-wrap">
            ...
            <button class="btn-categorias" id="btn-categorias" onclick="togglePanelMateriales()">
                <i class="fa-solid fa-recycle"></i> Categorías <span class="punto-activo" id="mat-indicador"></span>
            </button>

            <aside class="sidebar sidebar-der" id="panel-materiales">
                <div class="pm-head">
                    <h3 class="titulo-sec"><i class="fa-solid fa-recycle"></i> Categorías</h3>
                    <button class="pm-cerrar" onclick="togglePanelMateriales()" title="Cerrar"><i class="fa-solid fa-xmark"></i></button>
                </div>
...
<script>
    ...
    function togglePanelMateriales() {
        const panel = document.getElementById('panel-materiales');
        const abierto = panel.classList.toggle('abierto');
        // Mientras el panel está abierto, el botón flotante se oculta
        document.getElementById('btn-categorias').classList.toggle('oculto', abierto);
    }
    function construirMateriales() {
        ...
        // Puntito amarillo en el botón flotante cuando hay un filtro de material activo
        document.getElementById('mat-indicador').classList.toggle('on', !!materialActivo);
    }
